...
template <typename T, int spatial_dim, int nodes_per_element>
__device__ T compute_energy_for_element(const int *element_nodes, const T *xloc,
                                        const T *dof, int element_index,
...
template <typename T>
__global__ void energy_kernel(int num_elements, const int *element_nodes,
                              const T *xloc, const T *dof, T *total_energy)
{
  ...
  if (i < num_elements)
  {
    ...
    T energy_contrib =
        compute_energy_for_element<T, spatial_dim, nodes_per_element>(
            element_nodes, xloc, dof, i, num_quadrature_pts, C1, D1);
...
template <typename T>
T energy(int num_elements, const int *element_nodes, const T *xloc,
         const T *dof)
{
  T total_energy = 0.0;
  ...
  energy_kernel<T><<<gridSize, blockSize>>>(num_elements, element_nodes, xloc,
                                            dof, d_total_energy);
