...
namespace FEAnalysis
{
  ...
  static __device__ void get_element_dof(int nodes[], const T dof[],
                                         T element_dof[])
  {
  ...
  template <typename T>
  __global__ void energy_kernel(int element_nodes[], const T xloc[], const T dof[], T *total_energy, T *C1, T *D1)
  {
    ...
    if (threadIdx.x == 0)
    {
      printf("block %i, quad %i, energy %f, grad %f, element_dof %f  \n", element_idx, threadIdx.x, elem_energy, grad[0], element_dof[0]);
  ...
  static T body_energy(int num_elements, int element_nodes[], T xloc[], T dof[], const int num_nodes, T C1, T D1)
  {
    ...
    int *d_num_elements;
    cudaMalloc(&d_num_elements, sizeof(int));
    cudaMemcpy(d_num_elements, &num_elements, sizeof(int), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(d_dof, dof, sizeof(T) * num_elements * dof_per_node, cudaMemcpyHostToDevice);
    ...
    energy_kernel<<<num_blocks, threads_per_block>>>(d_element_nodes, d_xloc, d_dof, d_total_energy, d_C1, d_D1);
