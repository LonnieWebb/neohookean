...
std::string trim(const std::string &str)
{
  ...
  {
    return ""; // Return an empty string if only whitespace characters are found
  }
               ...
               std::vector<std::string> *node_set_names)
{
  ...
  while (getline(meshFile, line))
  {
    ...
    {
      std::istringstream iss(line); // Add a trailing comma to ensure the last token is parsed.
      std::string token;
